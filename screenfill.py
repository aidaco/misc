...
def splashscreen(terminal: Terminal, text: str, duration: float):
    ...
    bg = '\n'.join(''.join(' ' for x in range(terminal.width)) for y in range(terminal.height))
    print(terminal.clear + terminal.home + bg + terminal.move_y(terminal.height // 2) + terminal.center(text))
    time.sleep(duration)


def screenfill(terminal: Terminal, *, char: str = '●', steps: int = 50, duration: float = 0.5, func: Callable[[int, int, float], list[list[bool]]]):
    ...
    for etime in itersleep(duration, steps):
        ...
        output = '\n'.join(''.join(char if flag else ' ' for flag in row) for row in should_fill)
...
def circle(width: int, height: int, percent: float) -> list[list[bool]]:
    ...
    return [[math.sqrt(x**2 + y**2) <= radius for x in range(width)] for y in range(height)]
...
def topleft_to_bottomright(width: int, height: int, percent: float) -> list[list[bool]]:
    return [[((y / height) + (x / width)) / 2 < percent for x in range(width)] for y in range(height)]


def inverted(func: Callable[[...], list[list[bool]]]) -> Callable[[...], list[list[bool]]]:
...
def reversed(func: Callable[[...], list[list[bool]]]) -> Callable[[...], list[list[bool]]]:
    def reverse(width: int, height: int, percent: float, *args, **kwargs):
        return func(width, height, 1 - percent, *args, **kwargs)
    return reverse


def flippedx(func: Callable[[...], list[list[bool]]]) -> Callable[[...], list[list[bool]]]:
...
def flippedy(func: Callable[[...], list[list[bool]]]) -> Callable[[...], list[list[bool]]]:
...
if __name__ == "__main__":
    ...
    mod_combos = list(chain(*(combinations(modifiers, n) for n in range(1, len(modifiers) + 1))))
    with t.hidden_cursor():
        for base in funcs:
            splashscreen(t, f"{base.__name__}", 1)
            screenfill(t, func=base)
            for mods in mod_combos:
                splashscreen(t, f"{base.__name__} {':'.join(f.__name__ for f in mods)}", 1)
                func = base
                for mod in mods:
                    func = mod(func)
